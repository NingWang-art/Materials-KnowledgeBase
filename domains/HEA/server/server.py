"""
HEA RAG问答系统 MCP Server
提供基于RAG的高熵合金文献知识问答服务
"""
import argparse
import logging
...
# 导入HEA特定模块（config和prompts现在在server目录下）
# 支持直接运行和作为模块导入两种方式
try:
    from .config import (
    ...
    from domains.HEA.server.config import (
        SERVER_HOST, SERVER_PORT, LOG_LEVEL, MAX_WORKERS, LLM_API_TIMEOUT, LLM_MAX_RETRIES,
        DB_PATH, INDEX_PATH, METADATA_PATH, EMBEDDING_MODEL, EMBEDDING_DIM, DEEPSEEK_CONFIG, TOP_K
    )
    from domains.HEA.server.prompts import (
        LITERATURE_SUMMARY_SYSTEM_PROMPT,
        get_literature_summary_user_prompt
    )
    from domains.HEA.server.utils import read_literature_fulltext

# === ARG PARSING ===
def parse_args():
    parser = argparse.ArgumentParser(description="HEA RAG MCP Server")
...
mcp = CalculationMCPServer("HEAkb", port=args.port, host=args.host)


# === MCP TOOL ===
@mcp.tool()
async def query_heakb_literature(
    question: str,
    top_k: int = 5
) -> QueryResult:
    """
    📚 查询HEAkb高熵合金文献知识库，基于RAG技术进行文献检索和总结生成。

    🔍 功能说明:
    -----------------------------------
    本工具采用多阶段处理流程：
    1. 使用RAG向量检索技术从HEAkb文献库中检索top-k个最相关的文本片段
    ...
    query_heakb_literature(
        question="高熵合金中的相变机制和微观结构演化规律是什么？"
    )

    # 检索更多结果（可能找到更多相关文献）
    query_heakb_literature(
        question="FCC到HCP相变的条件和影响因素",
        top_k=10
    )
    """
    top_k = min(top_k, 20)
...
if __name__ == "__main__":
    logger.info("Starting HEAkb MCP Server...")
