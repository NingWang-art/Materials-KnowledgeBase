        ...
        async def get_table_fields(table_name: str):
            ...
            async with aiohttp.ClientSession() as session:
                if self.db_name == 'electrolyte_db' or self.db_name == 'polymer_db':
                    ...
                    fields = []
                    primary_fields = []
                    # 检查result['data']和result['data']['list']是否存在
                    if result.get('data') and result['data'].get('list'):
                        ...
                        # 如果字段信息表中没有记录，尝试直接查询表结构
                        import logging
                        logging.warning(f"表 {table_name} 在字段信息表中无记录，尝试直接查询表结构...")
                        try:
                            params = {'tableAk': table_name}
                            async with session.get(
                                self.table_url, headers=self.get_headers, params=params
                            ) as table_response:
                                table_result = await table_response.json()
                                if table_result.get('data') and table_result['data'].get('fields'):
                                    # 从表结构获取字段
                                    for field_info in table_result['data']['fields']:
                                        field_name = field_info.get('name', '')
                                        if field_name and not field_name.startswith('_') and not field_name.startswith('a1b2c3d4e5_'):
                                            fields.append(field_name)
                                    logging.info(f"从表结构获取到 {len(fields)} 个字段: {fields[:5]}...")
                                elif table_result.get('data') and table_result['data'].get('schema'):
                                    # 从schema获取字段
                                    for col in table_result['data']['schema']:
                                        field_name = col.get('dataIndex') or col.get('title', '')
                                        if field_name and not field_name.startswith('_') and not field_name.startswith('a1b2c3d4e5_'):
                                            fields.append(field_name)
                                    logging.info(f"从schema获取到 {len(fields)} 个字段: {fields[:5]}...")
                        except Exception as e:
                            logging.warning(f"查询表结构失败: {e}")
