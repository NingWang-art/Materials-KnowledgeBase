...
# Stainless Steel RAG Server URL (SSE endpoint)
# 格式: http://host:port/sse
server_url = os.getenv("STAINLESS_STEEL_SERVER_URL", "http://keld1409173.bohrium.tech:50009/sse")

# === 2. Initialize MCP tools for Stainless Steel Knowledge Base ===
...
root_agent = LlmAgent(
    ...
    name="stainless_steel_knowledgebase_agent",
    description="Advanced Stainless Steel literature knowledge base agent with RAG capabilities for comprehensive literature analysis, multi-document summarization, and in-depth research report generation.",
    instruction=(
        "You can call one MCP tool exposed by the Stainless Steel RAG server:\n\n"

        "=== TOOL: query_stainless_steel_literature ===\n"
        "Advanced RAG-based query tool for the Stainless Steel literature knowledge base.\n"
        "It supports:\n"
        ...
        "• Thousands of text chunks from Stainless Steel literature\n"
        ...
        "   - List all cited sources in numerical order [1], [2], [3], etc.\n"
        "   - For each reference, extract and include:\n"
        "     • Title (if available in the summary)\n"
        "     • Authors (if available in the summary)\n"
        "     • Journal/Publication (if available in the summary)\n"
        "     • DOI (if available in the summary)\n"
        "     • File ID or source identifier (if no other info available)\n"
        "   - Format example:\n"
        "     [1] Title: [title from summary]\n"
        "         Authors: [authors from summary]\n"
        "         Journal: [journal from summary]\n"
        "         DOI: [DOI from summary]\n"
        "   - If summary lacks detailed info, use: [1] Source: [file_id or relevant identifier from summary]\n\n"
    ),
    tools=[mcp_tools],
)
